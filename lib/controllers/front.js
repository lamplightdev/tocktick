...
class ControllerFront extends Controller {
  ...
  _initView(container, templates) {
    ...
    if (!this._view) {
      this._view = new ViewFront(container, templates, {
        ...
        onTimerDetailsClick: (timerID, onDone) => {
          this.timerDetailsClicked(timerID, onDone);
        }
      });
    }
  }
  ...
  removeTimer(timerData) {
    let timer = new Timer(timerData._members, timerData._id);
  ...
  timerDetailsClicked(timerID, container, onDone) {
