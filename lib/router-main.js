    ...
    return this.checkOfflineStatus().then((data) => {
      ...
      if(this._data.user) {
        ...
        socket.on('timerUpdated', timer => {
          this.controllers.front.addOrUpdateTimer(timer);
        ...
        socket.on('timerDeleted', timer => {
          this.controllers.front.removeTimer(timer);
      ...
      this.router = Router
        ...
        .add(/(?:$|\/(.*))/i, (preRendered, match) => {
          ...
          if (!this.controllers.front) {
            this.controllers.front = new ControllerFront(this._data, this._data.templates, document.getElementById('view'), {
              onTimerStartSubmit: (timer) => {
                console.log('timer started', timer);
                this.controllers.nav.getView().setDirty('nav-timer');
                this.controllers.nav.renderView();
