      ...
      this.router = Router
        .add(/account(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          if (!this.controllers.account) {
            this.controllers.account = new ControllerAccount(this._data, this._data.templates, document.getElementById('view-account'));
          }
        ...
        .add(/timers(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          if (!this.controllers.timers) {
            this.controllers.timers = new ControllerTimers(this._data, this._data.templates, document.getElementById('view-timers'), {
        ...
        .add(/(?:$|\/(.*))/i, (preRendered, match) => {
          ...
          if (!this.controllers.front) {
            this.controllers.front = new ControllerFront({
              ...
              timer: new Timer()
            }, this._data.templates, document.getElementById('view-front'), {
              onTimerStartSubmit: (timer) => {
                console.log('timer started', timer);
                this.controllers.nav.getView().setDirty('all');
                this.controllers.nav.renderView();
