...
var RouterProjects = require('./routers/projects');
...
var ControllerProjects = require('./controllers/projects');
...
class RouterMain {
  ...
  init() {
    return this.checkOfflineStatus().then((data) => {
      ...
      this.controllers.app = new ControllerApp(this._data, this._data.templates, document.getElementById('view'), {
      ...
      this.router = Router
        .add(/account(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          RouterAccount.match(match, this.getQueryParams(), (matched) => {
            ...
            this.controllers.nav.getView().setDirty('nav');
        ...
        .add(/projects(?:$|\/(.+))/i, (preRendered, match) => {

          this.controllers.nav.setData({
            current: 'projects'
          });

          if (!this.controllers.projects) {
            this.controllers.projects = new ControllerProjects(this._data, this._data.templates, document.getElementById('view'), {
          ...
          this._currentController = this.controllers.projects;

          this.controllers.app.addExtraData({
              view: this.controllers.projects.getViewData(),
              nav: this.controllers.nav.getViewData()
          });

          RouterProjects.match(match, this.getQueryParams(), (matched) => {
            this.controllers.projects.getView().setDirty('all');
            this.controllers.projects.renderView(preRendered);
        ...
        .add(/timers(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          this.controllers.timers.resetPaging();
