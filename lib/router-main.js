...
class RouterMain {
  ...
  init() {
    return this.checkOfflineStatus().then(data => {
      window._containerView = document.getElementById('view');
      ...
      this.controllers.app = new ControllerApp(this._data, this._data.templates, window._containerView, {
      ...
      this.router = Router
        .add(/account(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          if (!this.controllers.account) {
            this.controllers.account = new ControllerAccount(this._data, this._data.templates, window._containerView);
          }
        ...
        .add(/timers(?:$|\/(.+))/i, (preRendered, match) => {
          ...
          if (!this.controllers.timers) {
            this.controllers.timers = new ControllerTimers(this._data, this._data.templates, window._containerView, {
        ...
        .add(/(?:$|\/(.*))/i, (preRendered, match) => {
          ...
          if (!this.controllers.front) {
            this.controllers.front = new ControllerFront({
              ...
              timer: new Timer()
            }, this._data.templates, window._containerView, {
              onTimerStartSubmit: (timer) => {
                console.log('timer started', timer);
                this.controllers.nav.getView().setDirty('nav-timer');
                this.controllers.nav.renderView();
