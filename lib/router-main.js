...
class RouterMain {
  ...
  constructor(data) {
    ...
    if (savedData) {
      this._data.jobs = savedData.jobs;
      this._data.timers = savedData.timers;
    } else {
      Offline.saveLocal(this._data.jobs, this._data.timers);
    }

    this._data.grouped = Group.fromJSON(this._data.jobs, this._data.timers);
