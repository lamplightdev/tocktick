  ...
  render(data, preRendered) {
    ...
    if(this._isDirty.all) {
      if (!preRendered) {
        ...
        let newContainer = document.createElement(this._container.nodeName);
        newContainer.id = this._container.id;
        newContainer.className = this._container.className;
        newContainer.innerHTML = template(data);
        this._container = document.getElementById('view');
