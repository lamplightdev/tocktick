...
class ViewTimers extends View {
  ...
  initTimerDetails(data) {
    ...
    const buttons = this._container.querySelectorAll(".timer-list-item__edit .selectme");
    ...
    for (let i=0; i<buttons.length; i++) {
      ...
      button.addEventListener('click', (event) => {
        ...
        if (this._callbacks.onTimerDetailsClick) {
          this._callbacks.onTimerDetailsClick(timerID, this._container.querySelector('.timers-timer-edit'), (timer) => {
            console.log('timer edited (timers)', timer);
          });
        }
      });
    }
