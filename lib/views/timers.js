  ...
  initTimerEdit(data) {
    ...
    for (let i=0; i<buttons.length; i++) {
      ...
      button.addEventListener('click', () => {
        let form = this._container.querySelector('.timer-list-item--' + timerID + ' .timer-list-item__edit');
        if (form.classList.contains('timer-list-item__edit--show')) {
          form.classList.remove('timer-list-item__edit--show');
        } else {
          form.classList.add('timer-list-item__edit--show');
        }
      });
    }
  }
