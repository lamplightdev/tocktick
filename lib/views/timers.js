...
class ViewTimers extends View {
  ...
  initTimerDurationUpdate(data) {
    ...
    if (timers.length) {
      ...
      let timerSlice = timers.slice(data.timerPaging.current * data.timerPaging.length, data.timerPaging.length);

      for (let i=0 ; i<timerSlice.length; i++) {
        timerData.push({
          timer: timerSlice[i],
          $el: this._container.querySelector('.timer-list-item--' + timerSlice[i].getID() + ' .timer-list-item__timings__duration span'),
          lastDuration: 0
        });
      }
    ...
    this.registerContainerEventListener('submit', (event) => {
      if (event.target.classList.contains('timer-list-item__edit')) {
        ...
        if (data.grouped._timers.all[timerID]) {
          ...
          let tagIDs = [];
          for (let i=0; i<form.elements['tags[]'].length; i++) {
            const el = form.elements['tags[]'][i];
            if (el.checked) {
